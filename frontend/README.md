...
React SPA for the coinBaby finance app. Uses the FastAPI backend and Cognito for auth.

**Production:** [http://coinbaby.click](http://coinbaby.click) (S3 + CloudFront + Route 53)
...
To run the frontend as part of a full Docker setup (PostgreSQL + backend + frontend), see `notes/Deploy-local.md`. For AWS deploy (S3 + CloudFront + Route 53), see `notes/Deploy-AWS.md`.
...
- **Dashboard** (`/`) — 3-column layout: wallets/goals/budgets sidebar, Penny AI chat center, recent transactions sidebar.
